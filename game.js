...
class Board {
  ...
  display1() {
    let col_number = { a: 0, b: 1, c: 2, d: 3, e: 4, f: 5, g: 6, h: 7 };
    for (let r = 1; r <= 8; r++) {
      for (let f of ["a", "b", "c", "d", "e", "f", "g", "h"]) {
        let newobj = new Tile(f, r);
        if (this.#board[newobj.row][newobj.col].piece) {
          console.log(
            `Tile ${f}-${r} has --- ${
              this.#board[newobj.row][newobj.col].piece.id
            }`
          );
          console.log(
            `Tile ${f}-${r} Piece Color --- ${
              this.#board[newobj.row][newobj.col].piece.color
            }`
          );
        }
      }
...
class Piece {
  ...
  move(tile) {
    ...
    // Below if is checking that tile opted for moving piece should be a permitted tile
...
class Pawn extends Piece {
  ...
  get oneforward() {
    // console.log("Piece clicked ==== ", this.id);
    // console.log("Piece clicked ==== ", this.color);
  ...
  get left() {
    ...
    if (this.color == true) {
      for (let col = this.tile.col - 1; col >= 0; col--) {
        console.log("Piece clicked ==== ", this);
...
class Game {
  ...
  constructor(b) {
    if (b instanceof Board) this.#board = b;
  }
...
let board = new Board(boardposition);

game.board = board;

game.player1 = new Player("amit", true);

game.player2 = new Player("pradeep", false);
